...
describe RestaurantsController, "test" do
  ...
  context "Reject change" do
    ...
    it "case 1: not login" do
      xhr :post, "reject_change", :format => "js", id: @restaurant.slug
    ...
    it "case 3: login, admin" do
      ...
      sign_out @user1
      sign_in @admin
      restaurant_temp = RestaurantTemp.find_by_restaurant_id(@restaurant.id)
      # UserMailer.should_receive(:send_reject_restaurant_update).with(@user1.email, restaurant_temp)
      
      expect(@restaurant.schedules.count).to eq 7
      xhr :post, "reject_change", :format => "js", id: @restaurant.slug
      # expect { xhr :post, "reject_change", :format => "js", id: @restaurant.slug }.to change { ActionMailer::Base.deliveries.count }.by(1)

      expect(assigns[:restaurant].schedules.count).to eq 7
      expect(assigns[:restaurant].name.to_s).not_to eq @params_update["name"]
      expect(assigns[:restaurant].email.to_s).not_to eq @params_update["email"]
      expect(assigns[:restaurant].direction.to_s).not_to eq @params_update["direction"]
      expect(assigns[:restaurant].address.to_s).not_to eq @params_update["address"]
      expect(assigns[:restaurant].district.to_s).not_to eq @params_update["district"]
      expect(assigns[:restaurant].postcode.to_s).not_to eq @params_update["postcode"]
      expect(assigns[:restaurant].lat.to_s).not_to eq @params_update["lat"]
      expect(assigns[:restaurant].lng.to_s).not_to eq @params_update["lng"]
      expect(assigns[:restaurant].filter_ids).not_to eq (@params_update["filter_ids"] << @params_update["filter_id_shisha"] << @params_update["filter_id_price"])
      expect(assigns[:restaurant].halal_status.to_s).not_to eq @params_update["halal_status"]
